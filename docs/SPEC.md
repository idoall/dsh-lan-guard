...
### F9 设备批准与永久拉黑（2026-09-25 用户拍板方案 A）

「吊销」只能让设备的身份 cookie 立即失效；换浏览器用密码重新配对仍可进来。F9 让**拉黑变得永久**——不依赖设备指纹（指纹会因换浏览器/升级系统失效），而是把放行权交给管理员。

| 项 | 规格 |
| --- | --- |
| 设备状态 | `pending`（待批准）/ `approved`（已授权）/ `blocked`（已拉黑）三态；旧记录（无 status 字段）加载时**迁移为 `approved`**，不打断既有设备 |
| 开关 | `auth.requireApproval`（volatile，默认 **false**＝保持现状）：开启后新设备完成命名进入 `pending`，需管理员在设置页点「批准」才能访问 |
| 门禁 | `approved` → 放行；`pending` → 手机侧显示**等待批准**页（`/api/*` 返回 `403 pending_approval`）；`blocked` → `403`「已被移除访问权限」，且**重新配对仍被拒**（唯一出路是管理员「解除拉黑」） |
| 端点 | `POST /plugins/dsh-lan-guard/devices`：`action: approve \| block \| unblock \| revoke \| delete`（沿用既有管理面鉴权、CSRF 与远程只读规则） |
| 设置页 | 两个开关（命名确认 / 管理员批准）+ 三组列表：**待批准**（批准 / 拒绝并拉黑）、**已授权**（吊销并拉黑）、**已拉黑**（解除拉黑）；待批准非空时顶部黄色提醒条 |
| 不做 | 不做设备指纹、不做自动放行、不引入新依赖 |
...
**帧级状态提示（P4-f）——已于 2026-09-24 按用户要求移除**：曾向官方追加型 seat `shell.overlay` 注册一个右下角胶囊（「🌐 局域网访问已开启」+ 链接 + ✕）。用户指出两点：它把**免密链接（含 token）常驻在屏幕上**（截屏/共享屏幕会一并被拍到），且每次新会话都会再次出现。现**完全移除**——插件不再注册任何 `shell.overlay` 内容，状态改由设置页「扫码访问」tab 呈现。
